...
from graphsaint.tensorflow_version_2.inits import glorot,zeros,trained,ones,xavier,uniform
        ...
        with tf.compat.v1.name_scope(self.name):
            if self.logging:
                if type(inputs)==type([]) or type(inputs)==type((1,2)):
                    ...
                    _ip = inputs
                tf.compat.v1.summary.histogram(self.name + '/inputs', _ip)
...
class JumpingKnowledge(Layer):
    ...
    def _call(self, inputs):
        ...
        if not self.mode:
            ...
            feats_aggr =  tf.reduce_max(input_tensor=feats_stack,axis=0)
        else:
        ...
        vw = self.act(vw)
        if self.bias == 'norm':
            mean,variance = tf.nn.moments(x=vw,axes=[1],keepdims=True)
...
class HighOrderAggregator(Layer):
    ...
    def _call(self, inputs):
        ...
        for o in range(self.order):
            for a in range(o+1):
                ans1 = tf.sparse.sparse_dense_matmul(adj_norm,vecs_hop[o+1])
...
class AttentionAggregator(Layer):
    ...
    def _F_edge_weight(self,adj_part,vecs_neigh,vecs_self,offset=0):
        ...
        a1 = tf.SparseTensor(adj_mask.indices,tf.nn.embedding_lookup(params=vecs_neigh,ids=adj_mask.indices[:,1]),adj_mask.dense_shape)
        a2 = tf.SparseTensor(adj_mask.indices,tf.nn.embedding_lookup(params=vecs_self,ids=adj_mask.indices[:,0]+offset),adj_mask.dense_shape)
    ...
    def _call(self, inputs):
        ...
        vecs_do2 = tf.nn.dropout(vecs, 1 - (1-self.dropout))
        vw_self = tf.matmul(vecs_do2,self.vars['order0_weights'])
        ret_self = self.act(vw_self + self.vars['order0_bias'])
        if self.bias == 'norm':
            mean,variance = tf.nn.moments(x=ret_self,axes=[1],keepdims=True)
        ...
        for i in range(self.mulhead):
            ...
            vw_neigh_att.append(tf.reduce_sum(input_tensor=vw_neigh[i]*self.vars['attention_1_h{}'.format(i)],axis=-1)\
        ...
        for _adj in adj_partition_list:
            ...
            offset += dim0_adj_sub
        ret_neigh = tf.cond(pred=self.is_train, true_fn=lambda: tf.concat(ret_neigh_l_subg,axis=1), false_fn=lambda: tf.concat(ret_neigh_l_fullg,axis=0))
        if self.bias == 'norm':
            mean,variance = tf.nn.moments(x=ret_neigh,axes=[1],keepdims=True)
