...
class Minibatch:
    ...
    def __init__(self, adj_full_norm, adj_train, role, class_arr, placeholders,
                 train_params, **kwargs):
        """
    ...
    def set_sampler(self, train_phases):
        ...
        if self.method_sample == 'mrw':
            if 'deg_clip' in train_phases:
                ...
                # setting this to a large number so essentially there is no
                # clipping in probability
                _deg_clip = 100000
            self.size_subg_budget = train_phases['size_subgraph']
            self.graph_sampler = mrw_sampling(
                self.adj_train, self.node_train, self.size_subg_budget,
                train_phases['size_frontier'],_deg_clip
            )
        elif self.method_sample == 'rw':
            self.size_subg_budget = \
                train_phases['num_root']*train_phases['depth']
            self.graph_sampler = rw_sampling(
                self.adj_train, self.node_train, self.size_subg_budget,
                int(train_phases['num_root']), int(train_phases['depth'])
            )
        ...
        # For edge sampler, no need to estimate norm factors, we can calculate
        # directly. However, for integrity of the framework, we decide to
        # follow the same procedure for all samplers:
...
#            tf.compat.v1.SparseTensorValue(np.column_stack(adj_0.nonzero()),adj_0.data,adj_0.shape)})
#        feed_dict.update({self.placeholders['adj_subgraph_1']: \
#            tf.compat.v1.SparseTensorValue(np.column_stack(adj_1.nonzero()),adj_1.data,adj_1.shape)})
#        feed_dict.update({self.placeholders['adj_subgraph_2']: \
#            tf.compat.v1.SparseTensorValue(np.column_stack(adj_2.nonzero()),adj_2.data,adj_2.shape)})
#        feed_dict.update({self.placeholders['adj_subgraph_3']: \
#            tf.compat.v1.SparseTensorValue(np.column_stack(adj_3.nonzero()),adj_3.data,adj_3.shape)})
...
#        return feed_dict, self.class_arr[self.node_subgraph]
    ...
    def end(self):
        """ DONE """
        return (self.batch_num+1)*self.size_subg_budget >= \
            self.node_train.shape[0]
