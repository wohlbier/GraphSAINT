...
def uniform(shape, scale=0.05, name=None):
    """Uniform init."""
    initial = tf.random.uniform(shape, minval=-scale, maxval=scale, dtype=DTYPE)
...
def glorot(shape, name=None):
    ...
    initial = tf.random.uniform(shape, minval=-init_range, maxval=init_range, dtype=DTYPE)
    return tf.Variable(initial, name=name)

def xavier(shape, name=None):
    init_dev = np.sqrt(2.0/(shape[0]+shape[1]))
    initial = tf.random.normal(shape,mean=0,stddev=init_dev,dtype=DTYPE)
    return tf.Variable(initial, name=name)
...
def trained(val_array, name=None):
    initial = tf.convert_to_tensor(value=val_array)
    return tf.Variable(initial, name=name)
